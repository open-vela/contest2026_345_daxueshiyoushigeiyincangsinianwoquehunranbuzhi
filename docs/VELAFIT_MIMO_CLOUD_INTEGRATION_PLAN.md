...
## 3. 端侧本地关键词唤醒（KWS）设计与大赛规范

> [!IMPORTANT]
> **大赛官方统一唤醒词规范**：
> 按照《2026 openvela 大赛参赛须知与规则》第 6 条明确要求：
> **“若项目包含 AI 语音唤醒功能，统一使用指定唤醒词：你好，openvela / Hello，openvela。”**
> VelaFit 端侧 KWS 引擎原生实现并严格遵循该规则，支持 `你好，openvela`（主唤醒词）与 `Hello, openvela`（英文唤醒词）。

### 3.1 为什么关键词唤醒（KWS）必须留在本地端侧？
